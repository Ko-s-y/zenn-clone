  ...
  describe "GET api/v1/articles" do
    ...
    context "page を params で送信しない時" do
      ...
      it "1ページ目のレコード10件取得できる" do
        ...
        expect(res["articles"][0].keys).to eq ["id", "title", "content", "status_i18n", "created_at", "from_today", "user"]
    ...
    context "article_id に対応する articles レコードが存在する時" do
      ...
      context "articles レコードのステータスが公開中の時" do
        ...
        it "正常にレコードを取得できる" do
          subject
          res = JSON.parse(response.body)
          expect(res.keys).to eq ["id", "title", "content", "status_i18n", "created_at", "from_today", "user"]
